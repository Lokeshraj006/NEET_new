...
def _page_has_visual_content(page) -> bool:
    try:
        return bool(getattr(page, "images", None) or getattr(page, "rects", None) or getattr(page, "lines", None))
    except Exception:
        return False
...
def _render_pdf_page_image_base64(page, *, resolution: int = 180) -> str:
    if fitz is None:
        return ""
    try:
        doc = fitz.open(str(NEW_MOCK_TEST_PDF))
        try:
            page_index = max(0, int(getattr(page, "page_number", 1)) - 1)
            target = doc[page_index]
            pixmap = target.get_pixmap(dpi=resolution, alpha=False)
...
def _is_mock_test_answer_key_page(text: str) -> bool:
    lowered = text.lower()
    if not lowered.strip():
        return False
    letter_matches = re.findall(r"(?mi)\b\d{1,3}\s*[\.)]?\s*\(?[a-d]\)?", text)
    if "answer key" in lowered or "answers" in lowered:
        return len(letter_matches) >= 8
    return len(letter_matches) >= 24


@lru_cache(maxsize=2)
def _mock_test_pdf_page_texts(split_columns: bool = False) -> List[str]:
    if not NEW_MOCK_TEST_PDF.exists():
        return []

    texts: List[str] = []
    try:
        with pdfplumber.open(NEW_MOCK_TEST_PDF) as pdf:
            for page in pdf.pages:
                texts.append(_extract_pdf_page_text(page, split_columns=split_columns))
    except Exception:
        return []
    return texts
...
@lru_cache(maxsize=1)
def _mock_test_pdf_set_ranges() -> List[Dict[str, int]]:
    return [
        {"set_id": 1, "start_page": 1, "end_page": 20, "answer_page": 28},
        {"set_id": 2, "start_page": 48, "end_page": 71, "answer_page": 79},
        {"set_id": 3, "start_page": 98, "end_page": 118, "answer_page": 120},
        {"set_id": 4, "start_page": 146, "end_page": 167, "answer_page": 169},
        {"set_id": 5, "start_page": 1, "end_page": 20, "answer_page": 28},
        {"set_id": 6, "start_page": 48, "end_page": 71, "answer_page": 79},
        {"set_id": 7, "start_page": 1, "end_page": 20, "answer_page": 28},
        {"set_id": 8, "start_page": 48, "end_page": 71, "answer_page": 79},
    ]


def _parse_fixed_set_answers(answer_text: str) -> Dict[int, int]:
    answers: Dict[int, int] = {}
    
    # 1. Try legacy Q./A. row logic first
    q_lines = []
    a_lines = []
    for line in answer_text.splitlines():
        stripped = line.strip()
        if stripped.startswith("Q."):
            q_lines.append(stripped[2:].strip().split())
        elif stripped.startswith("A."):
            a_lines.append(stripped[2:].strip().split())
            
    if q_lines and a_lines:
        for q_row, a_row in zip(q_lines, a_lines):
            for q_num, ans in zip(q_row, a_row):
                try:
                    answers[int(q_num)] = int(ans) - 1
                except ValueError:
                    pass
        if answers:
            return answers
            
    # 2. Try explanation-based format: 1. Option (2) is correct
    matches = re.findall(r"(?mi)(\d{1,3})\.\s*Option\s*\(?([a-d1-4])\)?\s*is", answer_text)
    if len(matches) >= 50:
        for num_str, ans_str in matches:
            answers[int(num_str)] = _answer_token_to_index(ans_str)
        return answers
        
    # 3. Try table-based number format: 1 1 Electronic Devices or 74. 1 Properties
    matches = re.findall(r"\b(\d{1,3})\s*\.?\s+([1-4])\b", answer_text)
    if len(matches) >= 50:
        for num_str, ans_str in matches:
            num = int(num_str)
            if 1 <= num <= 200:
                answers[num] = int(ans_str) - 1
        return answers
        
    # 4. Try table-based letter format: 1 (b) 41 (a)
    matches = re.findall(r"\b(\d{1,3})\s*\(?([a-d])\)?\b", answer_text.lower())
    if len(matches) >= 50:
        for num_str, ans_str in matches:
            num = int(num_str)
            if 1 <= num <= 200:
                answers[num] = _answer_token_to_index(ans_str)
        return answers
...
def _extract_question_blocks(text: str) -> List[tuple[int, str]]:
    starts = list(re.finditer(r"(?m)^\s*(?:Q\.\s*)?(\d{1,3})\.\s+", text))
    question_starts = [(int(match.group(1)), match.start()) for match in starts if int(match.group(1)) >= 1]
...
def _parse_fixed_set_questions(page_entries: List[Dict], answers: Dict[int, int], set_id: int) -> List[Dict]:
    questions: List[Dict] = []
    ...
    for page_entry in page_entries:
        ...
        raw_text = str(page_entry.get("text") or "")
        
        # Clean text starting from first SECTION match to exclude instruction page
        match = re.search(r"(?i)SECTION\s*-\s*[A-D]", raw_text)
        if match:
            raw_text = raw_text[match.start():]
            
        page_text = _clean_fixed_set_text(raw_text)
        if not page_text:
            continue
        page_image = str(page_entry.get("image") or "")
        page_has_visuals = bool(page_entry.get("has_visuals")) and bool(page_image)
        question_starts = _extract_question_blocks(page_text)
        
        for number, block in question_starts:
            ...
            # Bilingual clean up
            mid_pattern = rf"\b{number}\.\s+"
            parts = re.split(mid_pattern, question_text)
            if len(parts) > 1:
                question_text = parts[-1].strip()
                
            question_text = _normalize_multiline_text(question_text)

            if not _looks_like_fixed_question(question_text, options):
                continue
            ...
            answer_index = int(answers.get(number, 0))
            questions.append({
...
def _split_bilingual_line(line: str) -> str:
    stripped = line.rstrip('\n').rstrip('\r')
    if not stripped:
        return ""
    matches = list(re.finditer(r"\b\d{1,3}\.\s+", stripped))
    if len(matches) > 1:
        return stripped[matches[-1].start():]
    elif len(matches) == 1:
        if matches[0].start() > 5:
            return stripped[matches[0].start():]
    last_dev_idx = -1
...
def _get_cleanliness_score(q: Dict) -> float:
    score = 0.0
    q_text = q.get("question", "")
    options = q.get("options", [])
    if len(options) == 4 and all(opt.strip() for opt in options):
        score += 20.0
        if all(len(opt.strip()) > 1 for opt in options):
            score += 10.0
    if "\x00" not in q_text and all("\x00" not in opt for opt in options):
        score += 10.0
    if not re.search(r"[\u0900-\u097F]", q_text) and not any(re.search(r"[\u0900-\u097F]", opt) for opt in options):
        score += 10.0
    q_len = len(q_text)
    if 40 <= q_len <= 300:
        score += 5.0
        ...
        score += 3.0
    if len(options) == 4:
        opt_lens = [len(opt) for opt in options]
        max_opt_len = max(opt_lens)
        if max_opt_len < 100:
            score += 5.0
    if q_text.strip() and q_text.strip()[-1] in "?:.":
        score += 5.0
    return score


def _filter_clean_35_questions(questions: List[Dict]) -> List[Dict]:
    ...
    for subj in by_subject:
        by_subject[subj].sort(key=_get_cleanliness_score, reverse=True)
    targets = {
        "Physics": 9,
        "Chemistry": 9,
        "Botany": 9,
    ...
    for subj, limit in targets.items():
        sub_list = by_subject[subj]
        selected_questions.extend(sub_list[:limit])
...
def _load_fixed_set_bundle(set_id: int) -> Dict:
    ...
    if paper_path.suffix.lower() == ".pdf":
        ...
        if not range_info:
            raise HTTPException(status_code=404, detail=f"Mock test set {set_id} not found in the uploaded PDF.")
        start_page = max(1, int(range_info["start_page"]))
        end_page = int(range_info["end_page"])
        ...
        try:
            with pdfplumber.open(paper_path) as pdf:
                for page_number in range(start_page, end_page + 1):
                    ...
                    page_text = _extract_pdf_page_text(page, split_columns=True)
                    ...
                    page_entries.append(
                        {
                            ...
                            "text": page_text,
                            "has_visuals": _page_has_visual_content(page),
                            "image": _render_pdf_page_image_base64(page),
                        }
                    )
                answer_page = int(range_info.get("answer_page") or 0)
                if 0 < answer_page <= len(pdf.pages):
                    if answer_page == 28:
                        ans_texts = [_extract_pdf_page_text(pdf.pages[p - 1], split_columns=True) for p in range(28, min(48, len(pdf.pages) + 1))]
                        answer_text = "\n".join(ans_texts)
                    elif answer_page == 79:
                        ans_texts = [_extract_pdf_page_text(pdf.pages[p - 1], split_columns=True) for p in range(79, min(98, len(pdf.pages) + 1))]
                        answer_text = "\n".join(ans_texts)
                    elif answer_page == 120:
                        ans_texts = [_extract_pdf_page_text(pdf.pages[p - 1], split_columns=True) for p in range(120, min(126, len(pdf.pages) + 1))]
                        answer_text = "\n".join(ans_texts)
                    else:
                        answer_text = _extract_pdf_page_text(pdf.pages[answer_page - 1], split_columns=True)
        except Exception:
            raise HTTPException(status_code=500, detail=f"Unable to parse mock test set {set_id}.")
        if not page_entries:
            raise HTTPException(status_code=500, detail=f"Unable to parse mock test set {set_id}.")
        ...
        raw_paper_text = paper_path.read_text(encoding="utf-8", errors="ignore")
        paper_lines = raw_paper_text.splitlines()
        cleaned_lines = [_split_bilingual_line(line) for line in paper_lines]
        paper_text = "\n".join(cleaned_lines)
        answer_path = paths.get("answer")
        answer_text = answer_path.read_text(encoding="utf-8", errors="ignore") if answer_path and answer_path.exists() else ""
        answers = _parse_fixed_set_answers(answer_text)
        page_entries = [
            {
                "page_number": 1,
...
def _load_text_docs() -> List[Dict[str, str]]:
    ...
    for file_path in sorted(DOCS_DIR.glob("*.txt")):
        if file_path.name in LEGACY_MOCK_TEST_DOCS:
            continue
        try:
    ...
    mock_test_text = _mock_test_pdf_text()
    if mock_test_text:
        docs.append({"source": NEW_MOCK_TEST_PDF.name, "text": mock_test_text})
    return docs
